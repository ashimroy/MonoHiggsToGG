...
//#include "MonoHiggsToGG/analysis/interface/LeptonSelection.h" //AR
	  ...
	  if (elvetoDipho.size()>0) {
		...
		if(vtxDipho.size()>0){
		  ...
		  if(massDipho.size()>0){
			...
			//for reference: selectLooseMuons( const std::vector<edm::Ptr<flashgg::Muon> > &muonPointers, Ptr<flashgg::DiPhotonCandidate> dipho, onst std::vector<edm::Ptr<reco::Vertex> > &vertexPointers, double muonEtaThreshold, double muonPtThreshold, double muPFIsoSumRelThreshold, double dRPhoLeadMuonThreshold, double dRPhoSubLeadMuonThreshold )
			//selectLooseMuons( theMuons->ptrs(), candDiphoPtr, primaryVertices->ptrs(), 2.4, 20., 0.25, 0.3, 0.3);  

			//for reference: selectMuons( const std::vector<edm::Ptr<flashgg::Muon> > Muons, Ptr<flashgg::DiPhotonCandidate> dipho, const std::vector<edm::Ptr<reco::Vertex>> vtx, double MuonPtCut, double MuonEtaCut, double MuonMiniIsoCut, double MuonPhotonDrCut, bool debug_, int id) 
			selectMuons( theMuons->ptrs(), candDiphoPtr, primaryVertices->ptrs(), 20., 2.4, 0.25, 0.3, 1);  // Included by AR 2023-08-26
